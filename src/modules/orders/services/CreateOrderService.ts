...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    const productsNonexistentIds = products.filter(
      product => !productsExistsIds.includes(product.id),
    );

    if (productsNonexistentIds.length) {
      throw new AppError(
        `Could not find products ${productsNonexistentIds[0].id}`,
      );
    }
    ...
    const findProductsWithNoQuantityAvailable = products.filter(
      product =>
        productsExists.filter(
          productsExist => productsExist.id === product.id,
    ...
    const orderedProductsQuantity = products.map(product => ({
      id: product.id,
      quantity:
        productsExists.filter(productExist => productExist.id === product.id)[0]
          .quantity - product.quantity,
    }));
